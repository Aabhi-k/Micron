...
async def auto_ingest_passman_bpd(db: AsyncSession) -> str | None:
    ...
    1. Checks if Passman BPD is already indexed in PostgreSQL.
    2. If already indexed, skips processing (0 startup overhead).
    3. If not, parses 21 pages via pypdf, chunks, embeds, and loads Qdrant and Postgres.
    """
    ...
    if existing_doc:
        logger.info(f"Auto-ingest: Passman BPD is already indexed (Document ID: {existing_doc.id}). Skipping.")
    ...
    try:
        ...
        if doc_id:
            logger.info(f"Auto-ingest: Successfully indexed Passman BPD (Document ID: {doc_id}) under tenant '{PASSMAN_TENANT_ID}'.")
            return str(doc_id)
        else:
